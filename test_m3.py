...
def get_args_parser():
    ...
    # pvt_mmt
    parser.add_argument('--pvt_mmt', default='', help='load from checkpoint')
...
def evaluate(model: torch.nn.Module, data_loader: Iterable, device: torch.device):
    ...
    for data_iter_step, (radar_data, pws_data, _) in enumerate(data_loader):
        ...
        # Forward pass
        z_hat = model(ctx=x, ts=z)[0].squeeze()  # Assuming model returns a tuple, take the first element
    ...
    np.savetxt('klch-test-M3.csv', combined_array, delimiter=',')
    
    print(f"Evaluation complete. Results saved to klch-test-M3.csv")
